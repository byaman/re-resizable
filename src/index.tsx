...
export class Resizable extends React.PureComponent<ResizableProps, State> {
  ...
  calculateNewSizeFromDirection(clientX: number, clientY: number) {
    ...
    if (hasDirection('right', direction)) {
      newWidth = original.width + ((clientX - original.x) * (typeof resizeRatio === 'number' ? resizeRatio : resizeRatio.x || 1)) / scale;
      if (lockAspectRatio) {
    ...
    if (hasDirection('bottom', direction)) {
      newHeight = original.height + ((clientY - original.y) * (typeof resizeRatio === 'number' ? resizeRatio : resizeRatio.y || 1)) / scale;
      if (lockAspectRatio) {
